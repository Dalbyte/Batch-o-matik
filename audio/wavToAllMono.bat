...
for /L %%i in (0,1,2) do (
  ...
  for %%B in (%Bitrates%) do (
    ...
    ffmpeg -i "%~nx1" -c:a !codec! -b:a %%B -ac 1 "!filename!_!codec!-CBR-bit%%~B-mono.!extension!"
  )
  ...
  if "!codec!" neq "libopus" (
    for %%Q in (%VBR_Quality%) do (
      ...
      ffmpeg -i "%~nx1" -c:a !codec! -q:a %%Q -ac 1 "!filename!_!codec!-VBR-quality%%Q-mono.!extension!"
    )
  )
)
